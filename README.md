...
```bash
python crawl_jobs.py --input data/companies.csv --output jobs_out.csv --log crawl_log.csv
cp jobs_out.csv client/public/data/jobs_out.csv
```
...
GitHub Actions runs `.github/workflows/daily-scrape.yml` once per day at `0 9 * * *` UTC and can also be started manually from the Actions tab. The workflow installs Python dependencies, runs the scraper, copies `jobs_out.csv` into `client/public/data/jobs_out.csv`, commits changed CSV data back to `main`, builds the Vite app, and deploys GitHub Pages.
